...
def process_analitico(saldo_type="atual"):
    ...
    # Identificar contas sintéticas no Betha (manter apenas a primeira ocorrência)
    df_betha['Primeira Ocorrência'] = ~df_betha.duplicated(subset='Máscara', keep='first')
    df_betha = df_betha[~df_betha['Primeira Ocorrência']]  # Manter apenas as ocorrências subsequentes
...
def process_sintetico():
    ...
    try:
        ...
        try:
            ...
            df_betha['Saldo Atual'] = df_betha['Saldo Atual'].apply(remover_sinal_negativo)
        ...
        try:
            ...
            # Salvar os resultados filtrados
            df_sem_correspondencia_betha.to_csv(
                "data/Mascaras_Sem_Correspondencia_Betha_Sintetico.csv", index=False, encoding="utf-8"
            )
            df_sem_correspondencia_tce.to_csv(
                "data/Mascaras_Sem_Correspondencia_TCE_Sintetico.csv", index=False, encoding="utf-8"
            )
...
def process_inicial():
    print("Iniciando processamento no modo Inicial...")
    ...
    # PEGAR O SEGUNDO TIPO SALDO QUE FICA 4 POSICOES DEPOIS DO PRIMEIRO
    #df_betha['Tipo Saldo'] = df_betha.iloc[:, df_betha.columns.get_loc("Tipo Saldo") + 4]
    df_betha_agrupado.rename(columns={
        'Saldo Anterio': 'Saldo Atual'
    }, inplace=True)
    # ADICAO PARA CONSIDERAR CREDITO E DEBITO
    df_betha['Saldo Ajustado'] = df_betha.apply(ajustar_saldo, axis=1)
    ...
    # Selecionar colunas relevantes para o DataFrame tratado
    colunas_tratadas_betha = ['Máscara Normalizada', 'Conta Corrente', 'Saldo Anterior', 'Saldo Ajustado']#,'Tipo Saldo Atual'
    df_betha_tratado = df_betha[colunas_tratadas_betha]

    ###### PARTE DO CÓDIGO PARA PROCURAR UMA MASCARA E CONTA ESPECIFICA
   # print ("DF BETHA PARA VER O TIPO SALDO")
  #  print (df_betha_tratado[['Máscara Normalizada', 'Saldo Atual', 'Saldo Ajustado', 'Tipo Saldo Atual']].head(6660))
    mascara_filtrada = "2.1.8.8.1.99.99"
    conta_corrente_filtrada = "15007000"

    resultado = df_betha_tratado[
        (df_betha_tratado['Máscara Normalizada'] == mascara_filtrada) &
        (df_betha_tratado['Conta Corrente'] == conta_corrente_filtrada)
    ][['Máscara Normalizada', 'Conta Corrente', 'Saldo Ajustado']]
    #print (resultado)
    ...
    # Agrupar por máscara e conta corrente e somar os saldos
    # Filtrar o resultado agrupado para verificar a soma específica
    resultado_agrupado = df_betha_agrupado[
        (df_betha_agrupado['Máscara Normalizada'] == mascara_filtrada) &
        (df_betha_agrupado['Conta Corrente'] == conta_corrente_filtrada)
    ]
    #print("Resultado agrupado:")
    ...
    df_tce_tratado = pd.read_csv('data/TCE_Tratado.csv', encoding='utf-8')
    resultado_agrupado = df_betha_tratado[
        (df_betha_tratado['Máscara Normalizada'] == mascara_filtrada) &
        (df_betha_tratado['Conta Corrente'] == conta_corrente_filtrada)
    ]
    #print("Resultado tratado:")
    #print(resultado_agrupado)
    resultado_agrupado = df_betha_agrupado[
        (df_betha_agrupado['Máscara Normalizada'] == mascara_filtrada) &
        (df_betha_agrupado['Conta Corrente'] == conta_corrente_filtrada)
    ...
    #print("Resultado agrupado:")
    #print(resultado_agrupado)
    ### ATÉ AQUI PARECE ESTAR CORRETO

    # Renomear as colunas de saldo para garantir consistência
    df_tce_tratado.rename(columns={'Saldo atual': 'Saldo_atual_TCE'}, inplace=True)
    #print(df_betha_tratado[['Máscara Normalizada', 'Conta Corrente','Saldo_atual_Betha']].head(6660))
    # Realizar a comparação
    df_comparacao = pd.merge(
        df_betha_tratado,
        df_tce_tratado,
    ...
    #print(df_comparacao[['Máscara Normalizada', 'Conta Corrente', 'Saldo_atual_Betha', 'Saldo_atual_TCE']].head(6620))
    # Calcular a diferença de saldos
    df_comparacao['Diferença de Saldo'] = df_comparacao['Saldo_atual_Betha'] - df_comparacao['Saldo_atual_TCE']
    ...
    # Salvar os resultados filtrados
    df_sem_correspondencia_betha.to_csv(
        "data/Mascaras_Sem_Correspondencia_Betha_Inicial.csv", index=False, encoding="utf-8"
    )

    df_sem_correspondencia_tce.to_csv(
        "data/Mascaras_Sem_Correspondencia_TCE_Inicial.csv", index=False, encoding="utf-8"
    )
    logging.info("Máscaras sem correspondência identificadas e salvas com sucesso.")
